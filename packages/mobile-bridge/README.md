...
插件同时拥有 Host 与 Client 两部分：Host 负责出站隧道、状态与窄屏样式路由；Client 通过 Harness 的 `settings.section` Slot 注册“移动连接”，用于编辑服务器地址、本地端口、加密口令、所有者邮箱、扫码邮箱二因子和自动连接策略，并同步显示配对二维码和六位配对码；保存配置后会等待新票据就绪，未配对时，一次性票据会在过期前自动续签。插件不提供独立 HTML 配置面板。
...
The package owns both halves of the capability: the Host provides the outbound tunnel, status route, and narrow-screen stylesheet; the Client registers Mobile Bridge through the Harness `settings.section` Slot. The settings section edits the server URL, local port, optional passphrase, owner email, scan-time email second factor, and connection policy, and displays the pairing QR with its six-character pairing code; a save waits for the fresh ticket, and an unused one-time ticket renews before it expires. No standalone HTML configuration panel is shipped.
