...
The plugin never returns decrypted credentials to browser state or model tools. A user can run one bounded command directly from the host detail pane; model-initiated `ssh_execute_command` always uses the DSH approval service and executes only after an `allowed-once` decision. Double-clicking a host opens a long-lived xterm terminal with replay, input, resize, reconnect-safe detach, and explicit close. Jump-host chains use direct-tcpip forwarding with independent credential and known-host verification at every hop.
