...
服务器始终为每个在线桌面保留一张未消费的五分钟一次性票据。票据过期或被一台手机消费后，Host 使用私有 refresh token 立即签发下一张；二维码和六位码因此持续可用，且轮换不会改变稳定 bridge id 或已配设备的 E2EE key。
...
The server keeps one unconsumed five-minute ticket for every online desktop. Expiry or a successful pairing makes the Host issue the next ticket with its private refresh token. The QR and six-character code therefore remain available without changing the stable bridge id or existing device keys.
