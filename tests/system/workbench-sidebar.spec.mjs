...
test('SSH Better Sidebar inventory sanitizes handoff and drives xterm input and resize', async ({ page }) => {
  ...
  await expect(root.getByText(/主机资源管理器|Host explorer/, { exact: true })).toHaveCount(0)

  const edgeHost = root.getByRole('button', { name: /Edge gateway.*deploy@edge[.]internal[.]example:2222/ })
  await edgeHost.click({ button: 'right' })
  const hostMenu = page.getByRole('menu', { name: /Edge gateway.*操作|Actions for Edge gateway/ })
  await expect(hostMenu).toBeVisible()
  expect(await hostMenu.evaluate(element => element.parentElement?.tagName)).toBe('BODY')
  await expect(hostMenu.getByRole('menuitem', { name: /打开新终端|Open new terminal/ })).toBeVisible()
  await expect(hostMenu.getByRole('menuitem', { name: /测试连接|Test connection/ })).toBeVisible()
  await expect(hostMenu.getByRole('menuitem', { name: /编辑主机|Edit host/ })).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(hostMenu).toHaveCount(0)

  const cluster = root.getByRole('button', { name: /^Production edge 1$/ })
  await cluster.focus()
  await page.keyboard.press('Shift+F10')
  const clusterMenu = page.getByRole('menu', { name: /Production edge.*操作|Actions for Production edge/ })
  await expect(clusterMenu.getByRole('menuitem', { name: /编辑集群|Edit cluster/ })).toBeVisible()
  await page.keyboard.press('Escape')
...
test('API Better Sidebar inventory sanitizes handoff and renders executed response', async ({ page }) => {
  ...
  await expect(page.getByRole('listbox', { name: /环境|Environment/ })).toBeVisible()
  await page.keyboard.press('Escape')
  const collectionMenuButton = root.getByRole('button', { name: /Users.*(操作|actions)/ })
  await collectionMenuButton.click()
  const collectionMenu = page.getByRole('menu', { name: /Users.*(操作|actions)/ })
  await expect(collectionMenu).toBeVisible()
  const menuBox = await collectionMenu.boundingBox()
  expect(menuBox).not.toBeNull()
  expect(menuBox.x).toBeGreaterThanOrEqual(0)
  expect(menuBox.x + menuBox.width).toBeLessThanOrEqual(page.viewportSize().width)
  expect(await collectionMenu.evaluate(element => element.parentElement?.tagName)).toBe('BODY')
  await page.keyboard.press('Escape')
