...
- `@sparkelf/dsh-dataops-managed`：DataOps托管workspace的只读Settings呈现插件，说明身份、权限和DataOps工具由工作区管理；不读取账号、不持有credential，也不提供连接或断开操作。
...
- `@sparkelf/dsh-dataops-managed`: the read-only Settings presentation for DataOps-managed workspaces. It explains workspace ownership of identity, permissions, and DataOps tools without reading an account, holding credentials, or exposing connect and disconnect actions.
