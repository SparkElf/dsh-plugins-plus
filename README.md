...
- `@sparkelf/dsh-mineru`：独立的PDF解析工具。它直接读取official file upload向模型提供的只读路径并调用MinerU，不接管附件上传、prompt或Client UI。
- `@sparkelf/dsh-officecli`：把官方 OfficeCLI 二进制接入单入口 `officecli` 工具；结构化 argv 原样执行，直接读写原始 `.docx/.xlsx/.pptx`，不另造预览链路。
- `@sparkelf/dsh-office-viewer-fonts`：向 Better Sidebar Office Viewer 提供政务字体资产和 OfficeCLI 政务文档 Skill，不依赖 Univer。
...
- `@sparkelf/dsh-mineru`: an independent PDF parsing tool. It reads the official file-upload path shown to the model and calls MinerU without owning attachment intake, prompt admission, or Client UI.
- `@sparkelf/dsh-officecli`: exposes the official OfficeCLI binary through one `officecli` tool. Structured argv runs unchanged against original `.docx/.xlsx/.pptx` files, with no second preview path.
- `@sparkelf/dsh-office-viewer-fonts`: provides government-document font assets and an OfficeCLI government-document Skill to Better Sidebar Office Viewer, with no Univer dependency.
